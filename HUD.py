...
class hud:
    def __init__(self):
        ...
        SETTINGS.canvas_actual_width, SETTINGS.window_height - SETTINGS.canvas_target_height))
    ...
    def render(self, canvas):
        ...
        if SETTINGS.current_gun and SETTINGS.current_gun.ammo_type:
            self.text[2].update_string(
                '%s / %s' % (SETTINGS.current_gun.current_mag, SETTINGS.held_ammo[SETTINGS.current_gun.ammo_type]))
        else:
            self.text[2].update_string('-- / --')
