...
class Music:
    ...
    def control_music(self):
        if SETTINGS.music_volume > 0:
            if [x for x in gamestate.npcs.npc_list if x.state == npc_state.ATTACKING and not x.dead] and not SETTINGS.menu_showing or gamestate.player.player_states['dead']:
